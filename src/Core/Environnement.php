...
//    }
    public function getPathPublicAbsolute($fileName, $folder = ''){
        if(!defined('PATH_ROOT') || !defined('PATH_PUBLIC')){
            throw(new Exception("Configuration de l'environnement incomplète"));
        }
        if($folder != '') {
            $folder = $folder . '/';
        }
        return PATH_ROOT . '/' . PATH_PUBLIC . '/' . $folder . $fileName;
    }
